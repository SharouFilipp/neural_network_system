...
Проект построен на чистой архитектуре с четким разделением ответственности:
1.  **Domain Layer (Models):** Определяет бизнес-сущности (модели, пользователи, настройки).
2.  **Application Layer(Controllers):** Реализует бизнес-логику и сценарии использования
3.  **Infrastructure Layer (Repository, Strategies):** Обеспечивает хранение и алгоритмы обучения.
4.  **Presentation Layer (Web Interface):** Предоставляет REST API и веб-интерфейс
